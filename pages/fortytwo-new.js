...
class Player {
    ...
    chooseDomino(trickState) {
        ...
        } else if (this.role === PlayerRole.SETTER1 || this.role === PlayerRole.SETTER2) {
...
$(function() {
    ...
    // Update player names to use songbird names
    game.players[0].name = SONGBIRD_NAMES[0];
    game.players[1].name = SONGBIRD_NAMES[1];
    ...
    function ensureOpenBtn(openBtnId, openBtnLabel, showFn) {
        if ($('#' + openBtnId).length === 0) {
            const $btn = $(`<button id="${openBtnId}">${openBtnLabel}</button>`);
            $btn.css({
                position: 'absolute',
                left: '50%',
                top: '70vh',
    ...
    function showTableWindow() {
        makeDraggableWindow('table-window', 'Table', '(Table content coming soon)', 'open-table-btn', 'Open Table', {top: '80px', left: '60vw'});
    }
    ensureOpenBtn('open-table-btn', 'Open Table', showTableWindow);
    
    // Scorecard window
    function showScorecardWindow() {
        makeDraggableWindow('scorecard-window', 'Hand Scorecard', '(Scorecard content coming soon)', 'open-scorecard-btn', 'Open Scorecard', {top: '320px', left: '62vw'});
    }
    ensureOpenBtn('open-scorecard-btn', 'Open Scorecard', showScorecardWindow);
    
    // Overall Score window
    function showOverallScoreWindow() {
        makeDraggableWindow('overallscore-window', 'Overall Score', '(Overall score content coming soon)', 'open-overallscore-btn', 'Open Overall Score', {top: '560px', left: '64vw'});
    }
    ensureOpenBtn('open-overallscore-btn', 'Open Overall Score', showOverallScoreWindow);
    
    // Game controls window
    function showGameControlsWindow() {
        const content = `
            <div>
                <h3>Game Controls</h3>
                <div style="margin: 10px 0;">
                    <button id="start-bidding-btn">Start Bidding</button>
                </div>
                <div style="margin: 10px 0;">
                ...
                <div id="game-status">
                    ...
                    <p><strong>Current Phase:</strong> <span id="phase-text">${game.currentPhase}</span></p>
                </div>
            </div>
        `;
        makeDraggableWindow('controls-window', 'Game Controls', content, 'open-controls-btn', 'Game Controls', {top: '100px', left: '20px'});
        
        // Bind control events
        $('#start-bidding-btn').on('click', startBidding);
        $('#play-trick-btn').on('click', playTrick);
        $('#new-hand-btn').on('click', newHand);
    }
    ensureOpenBtn('open-controls-btn', 'Game Controls', showGameControlsWindow);
    ...
    function playTrick() {
        if (game.currentPhase !== 'playing') {
            alert('Game must be in playing phase to play tricks');
            return;
        }
        game.playTrick();
    }
    ...
    function newHand() {
        ...
        updateGameStatus();
        $('#game-area').empty();
        $('.draggable-window').remove();
        $('[id^="open-"]').hide();
    }
    
    function updateGameStatus() {
        $('#status-text').text(game.currentPhase);
        $('#phase-text').text(game.currentPhase);
    }
    ...
    function updateTableWindow() {
        const content = `
            <div style="padding: 10px;">
                <h3>Game State</h3>
                <p><strong>Phase:</strong> ${game.currentPhase}</p>
                <p><strong>Trump:</strong> ${game.trump !== null ? game.trump : 'Not set'}</p>
                <p><strong>Current Player:</strong> ${game.players[game.currentPlayerIndex].name}</p>
                <p><strong>Tricks Played:</strong> ${game.currentHandTricks.length}</p>
                <hr>
                <h4>Player Hands</h4>
                <p><strong>Human:</strong> ${game.players[0].hand.length} dominoes</p>
        ...
        // Update the table window content
    ...
    // Function to update scorecard window with real game data
    function updateScorecardWindow() {
        const content = `
            <div style="padding: 10px;">
                <h3>Hand Scorecard</h3>
                <p><strong>Us:</strong> ${game.usHandPoints} points</p>
                <p><strong>Them:</strong> ${game.themHandPoints} points</p>
                <hr>
                <h4>Current Hand</h4>
                <p><strong>Human Hand:</strong></p>
                <ul>
                    ${game.players[0].hand.map(d => `<li>${d.toString()}</li>`).join('')}
                </ul>
                ${game.trump !== null ? `<p><strong>Trump:</strong> ${game.trump}</p>` : ''}
            </div>
        `;
        
        // Update the scorecard window content
        const $scorecardWindow = $('#scorecard-window');
        if ($scorecardWindow.length > 0) {
            $scorecardWindow.find('.window-content').html(content);
        }
    }
    
    // Function to update overall score window with real game data
    function updateOverallScoreWindow() {
        const content = `
            <div style="padding: 10px;">
                <h3>Overall Score</h3>
                <p><strong>Us:</strong> ${game.scores[0]} points</p>
                <p><strong>Them:</strong> ${game.scores[1]} points</p>
                <hr>
                <h4>Teams</h4>
                <p><strong>Us:</strong> ${game.players[0].name} & ${game.players[2].name}</p>
                <p><strong>Them:</strong> ${game.players[1].name} & ${game.players[3].name}</p>
            </div>
        `;
        
        // Update the overall score window content
        const $overallScoreWindow = $('#overallscore-window');
        if ($overallScoreWindow.length > 0) {
            $overallScoreWindow.find('.window-content').html(content);
        }
    }
    
    $('#shuffle-btn').on('click', function() {
        // Remove any existing domino windows
        $('#game-area').empty();
        ...
        for (let i = 0; i < humanHand.length; i++) {
            const domino = humanHand[i];
            const $domino = $('<div class="domino-window"></div>');
            $domino.data('flipped', false);
            $domino.data('domino', domino); // Store the actual domino object
            
            // Create a more visually appealing domino representation
            const $dominoVisual = $('<div class="domino-visual"></div>');
            $dominoVisual.css({
                width: '60px',
                height: '40px',
                background: '#fff',
            ...
            // Add dots to represent domino values
            const leftValue = domino.ends[0];
            const rightValue = domino.ends[1];
            
            const $leftDots = $('<div class="left-dots"></div>');
            const $rightDots = $('<div class="right-dots"></div>');
            
            // Create dot representation (simplified)
            $leftDots.html('●'.repeat(leftValue));
            $rightDots.html('●'.repeat(rightValue));
            ...
            $dominoVisual.append($leftDots);
            $dominoVisual.append($rightDots);
            $domino.append($dominoVisual);
            
            // Placeholder audio (use a real file if available)
            const audioSrc = 'assets/audio/ch1/A' + ((i % 10) + 1) + '.mp3';
            const $audio = $('<audio preload="auto"><source src="' + audioSrc + '" type="audio/mpeg"></audio>');
            $domino.append($audio);
            
            $domino.append('<span class="domino-label">' + domino.toString() + '</span>');
            
            const $flipBtn = $('<button class="flip-btn">Flip</button>');
            $flipBtn.css({
                position: 'absolute',
                top: '4px',
                right: '4px',
            ...
            $flipBtn.on('click', function(e) {
                e.stopPropagation();
                const flipped = !$domino.data('flipped');
                $domino.data('flipped', flipped);
                
                // Use the real flip logic for modal dominoes
                if (flipped && game.trump !== null && Domino.isModal(domino, game.trump)) {
                    const flippedDomino = Domino.flipModal(domino);
                    $domino.data('domino', flippedDomino);
                    $domino.find('.domino-label').text(flippedDomino.toString() + ' (flipped)');
                    
                    // Update visual representation
                    const newLeftValue = flippedDomino.ends[0];
                    const newRightValue = flippedDomino.ends[1];
                    $leftDots.html('●'.repeat(newLeftValue));
                    $rightDots.html('●'.repeat(newRightValue));
                } else {
                    $domino.find('.domino-label').text(domino.toString() + (flipped ? ' (flipped)' : ''));
                }
            });
            $domino.append($flipBtn);
            
            // Add play button
            const $playBtn = $('<button class="play-btn">Play</button>');
            $playBtn.css({
                position: 'absolute',
                bottom: '4px',
                right: '4px',
            ...
            $domino.on('click', function() {
                $('.domino-window').removeClass('selected');
                $(this).addClass('selected');
                
                // Pause any other audio
                if (currentAudio && currentAudio !== $audio[0]) {
                    currentAudio.pause();
                    currentAudio.currentTime = 0;
                }
                
                // Play this domino's audio
                $audio[0].currentTime = 0;
                $audio[0].play();
                currentAudio = $audio[0];
            });
            
            $('#game-area').append($domino);
        }
        
        // Show all windows
        showTableWindow();
        showScorecardWindow();
        showOverallScoreWindow();
        showGameControlsWindow();
        
        // Update window content with real game data
        updateTableWindow();
    ...
    function playSelectedDomino($dominoElement) {
        const domino = $dominoElement.data('domino');
        if (!domino) {
        ...
        if (game.currentPhase !== 'playing') {
            alert('Game must be in playing phase to play dominoes');
            return;
        }
